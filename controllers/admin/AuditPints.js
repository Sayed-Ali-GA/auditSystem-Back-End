...
router.get("/audit-points", verifyToken, async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT
        ap.AuditPointID, ap.AuditComment, ap.SubPointCriteria,
        ap.Weightage, ap.RiskMatrix,
        mc.MajorCriteriaID, mc.MajorCriteriaName
      FROM AuditPoints ap
      LEFT JOIN MajorCriteria mc ON ap.MajorCriteriaID = mc.MajorCriteriaID
...
router.post("/audit-point", verifyToken, async (req, res) => {
  const { MajorCriteriaID, auditComment, subPointCriteria, weightage, riskMatrix } = req.body;
  try {
    const result = await pool.query(
...
router.put("/audit-points/:id", verifyToken, async (req, res) => {
  ...
  try {
    const update = await pool.query(
      `
      UPDATE AuditPoints
      SET MajorCriteriaID = $1, auditComment = $2, subPointCriteria = $3,
          weightage = $4, riskMatrix = $5
      WHERE AuditPointID = $6
      RETURNING AuditPointID;
      `,
      [MajorCriteriaID, auditComment, subPointCriteria, weightage, riskMatrix, id]
    );
