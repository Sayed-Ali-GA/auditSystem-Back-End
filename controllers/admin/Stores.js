...
router.post("/Stores", verifyToken, isAdmin, async (req, res) => {
  const { StoreCode, BrandID, LocationID, OpsManagerID, StoreManagerID } = req.body;

  try {
    const result = await pool.query(
      `INSERT INTO Stores (StoreCode, BrandID, LocationID, OpsManagerID, StoreManagerID)
       VALUES ($1, $2, $3, $4, $5) RETURNING StoreSerial`,
...
router.put("/Stores/:id", verifyToken, isAdmin, async (req, res) => {
  ...
  try {
    await pool.query(
      `UPDATE Stores
       SET StoreCode = $1, BrandID = $2, LocationID = $3, OpsManagerID = $4, StoreManagerID = $5
       WHERE StoreSerial = $6`,
